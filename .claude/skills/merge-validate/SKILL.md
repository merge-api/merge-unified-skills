...
description: |
  ...
  or after completing /merge-unified:merge-onboarding. Runs diagnostic checks against
...
Run diagnostic checks against a developer's Merge integration and output a pass/fail report. This skill chains naturally after `merge-onboarding` — once a developer has set up their Linked Account, this skill confirms everything actually works.
...
- After `/merge-unified:merge-onboarding` completes, proactively suggest this skill

Do NOT activate for:
- Initial setup (route to `merge-onboarding`)
