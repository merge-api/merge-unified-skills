---
name: merge-unified-link-implement-backend
...
- `merge-unified-link-setup-database` complete (`linked_accounts` table exists with `end_user_origin_id`, `account_token`, `integration_slug`, `category`, `status` columns)
...
If invoked from `merge-unified-implementing-link`, these were answered in Step 1d — use that context. Otherwise, ask the user now before proceeding.
...
Check if the SDK is already installed before adding it. Do not add a dependency that's already present. For per-language initialize / list / paginate samples, see `../merge-unified-onboarding/references/sdk-quickstarts.md`.
