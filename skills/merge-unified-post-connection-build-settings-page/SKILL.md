---
name: merge-unified-post-connection-build-settings-page
description: >
...
- All four backend endpoints from `merge-unified-link-implement-backend` working — these are routes on **your** server (e.g., `/api/merge/create-link-token`), not Merge API endpoints
...
If invoked from `merge-unified-implementing-post-connection`, these were answered in Step 1 — use that context. Otherwise, gather them now:
