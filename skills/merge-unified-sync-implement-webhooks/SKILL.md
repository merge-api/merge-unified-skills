---
name: merge-unified-sync-implement-webhooks
description: >
  ...
  also add `merge-unified-sync-implement-polling` as a fallback safety net.
license: MIT
metadata:
  author: Merge
  version: 0.3.0
...
> **Webhooks are PRIMARY; polling is a fallback.** For production reliability, also implement `merge-unified-sync-implement-polling` as a safety net — webhook delivery can lag, drop, or your endpoint can be briefly unavailable. The two complement each other.
...
> I'm the merge-unified-sync-implement-webhooks skill. I'll wire up a single webhook endpoint that handles both initial and subsequent Merge syncs, with HMAC verification and async processing. For production, plan to also run `merge-unified-sync-implement-polling` as a fallback.
...
- **Public webhook URL**: production URL, staging URL, or local tunnel (ngrok / cloudflared)? Confirm one is available before continuing — without a reachable URL, polling is the only option (see `merge-unified-sync-implement-polling`).
...
- **Add polling as a fallback** (`merge-unified-sync-implement-polling`): webhook delivery occasionally lags or drops. Polling on a slow cadence catches what webhooks miss.
